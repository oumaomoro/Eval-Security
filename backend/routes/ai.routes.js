...
router.post('/analyze-clause', authenticateToken, async (req, res) => {
  try {
    const { text, jurisdiction, sector } = req.body;

    if (!text || text.length < 20) {
      return res.status(400).json({ error: 'Text too short for analysis.' });
    }

    console.log(`[Word Add-in] Analyzing snippet for ${jurisdiction || 'Global'} / ${sector || 'General'} risk...`);

    // Use the jurisdictional analyzer
    const analysisResponse = await AnalyzerService.analyze(text, {
